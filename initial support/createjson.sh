...
path=/home/cr/                                                         # Here you will need to specify the path to the crDroid source code folder
...
zip=$(basename $path/out/target/product/$device/crDroidAndroid-11.0-*-$device-*-Vanilla.zip)        # Here we get the package name with the extension .zip
...
gh release create $nozip --notes "Automated release crDroid (Vanilla) for $device $version $date/$time" $path/out/target/product/$device/$zip
